...
def get_texts(dataset, dataset_args):
    ...
    # add the language code to the start of texts (<2en>)
    for config in dataset_args["dataset_configs"]:
        for field in dataset_args["dataset_fields"]:
            if isinstance(texts[config][field][0], list):
                texts[config][field] = [
                    [f"<2en> {text}" for text in texts]
                    for texts in texts[config][field]
                ]
            else:
                texts[config][field] = [
                    f"<2en> {text}"
...
def translate_texts(dataset, texts, translate_args, dataset_args):
    ...
    for config in dataset_args["dataset_configs"]:
        ...
        for field in dataset_args["dataset_fields"]:
            ...
            translations[config][field] = translate_madlad.main(
